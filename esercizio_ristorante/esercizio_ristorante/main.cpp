...
void bubblesort(List<Item>& L) {
	for (int i = 0; i < (L.length() - 1); i++) {
		for (int j = (L.length() - 1); j > i; j--) {
			...
			if (it_jj.getRicetta().numero > it_j.getRicetta().numero) //crescente
			{
				L.moveToPos(j);
		...
		while (getline(myfile, codice, ';')) {
			...
			for (prodotti.moveToStart(); prodotti.currPos() < prodotti.length();) {
				...
				if (p.getCodice() == codice) {
					...
					prodotti.next(); // faccio next qui perche remove() fa next in automatico
				}
			}
	...
	while (true) {
		...
		for (prodotti.moveToStart(); prodotti.currPos() < prodotti.length();) {
			...
			if (p.getCodice() == r.ingrediente1) {
				...
				prodotti.next(); // in questo caso il next serve perche faccio anche insert e quindi il puntatore rimane bloccato.
			}
