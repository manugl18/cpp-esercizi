...
int main(int argc, char** argv) {
	...
	{
		if (cur_time_to_complete == 0) // vuol dire che il task corrente ha finito e bisogna toglierlo dalla coda
		{
			Item completed = pq.getmax();
			cur_max_priority = 0;
			cur_time_to_complete = -1;
			cout << "time=" << time << " completed task: " << completed << endl;
			if (!pq.empty()) // se  non è vuota ci sono altri task che stanno aspettando
			{
				// tolgo il dato, leggo e inserisco
				Item max = pq.getmax();
		...
		if (rand() % 100 < 30) // se < 30 arriva un nuovo processo
		{
			int priority = (int)rand() % 100 + 1;
			int total_time = (int)rand() % 10 + 1;
			Item newitem(priority, total_time);
			if (priority > cur_max_priority) // se la prorieta del nuovo task è maggiore del vecchio task, quello in eseguzione si deve interrompere
			{
				if (!pq.empty())
				{
					// tiriamo fuori quello vecchio in esecuzione, leggo e inserisco
					Item max = pq.getmax();
