...
/* Algoritmo per inserimento level-order (in ampiezza)

   Se il nodo radice è nullo inserisci in radice e termina
   Crea una coda e inserisci il nodo radice nella coda
   Ripeti finchè la coda non è vuota
		estrai un elemento dalla coda	
		se il figlio sinistro dell'elemento estratto dalla coda è vuoto
			inserisci il nodo come figlio sinistro dell'elemento estratto dalla coda e ritorna
		altrimenti
			inserisci nella coda il nodo figlio sinistro dell'elemento estratto dalla coda 
		se il figlio destro dell'elemento estratto dalla coda è vuoto
			inserisci il nodo come figlio destro dell'elemento estratto dalla coda e ritorna
		altrimenti
			inserisci nella coda il nodo figlio destro dell'elemento estratto dalla coda
   
*/	 
	void insert_levelorder(T newData, Node<T>* &theRoot)
	{
		if (theRoot == NULL)
		{
			theRoot = new Node<T>(newData);
		...
		{
			tempNode = q.dequeue();
			if (tempNode->lChildptr == NULL)
			{
				tempNode->lChildptr = new Node<T>(newData);
				return;
			}
			else
			{
    ...
    public:
		...
		void AddItem_levelorder(T newData)
		{
			insert_levelorder(newData, root);
		}
