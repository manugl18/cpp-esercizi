...
private:
	...
	void traverse_levelorder_NR(Node<T>* theRoot)
	 ...
	 Node<T>* max(T a[], int l, int r)
	  ...
	  int altezza_minimale(Node<T>* h)
	  { 
		  if (h == NULL) 
			  return -1;
		  else
		  {
		   int altezza_min_l = altezza_minimale(h->lChildptr), altezza_min_r = altezza_minimale(h->rChildptr);
		   
           if ((h->lChildptr==NULL) && (h->rChildptr != NULL))
           {
               cout << "altezza minimale " << h->data.key() << " " << altezza_min_r + 1 << endl;
               return altezza_min_r + 1;
           }
           else if ((h->lChildptr != NULL) && (h->rChildptr == NULL))
           {
               cout << "altezza minimale " << h->data.key() << " " << altezza_min_r + 1 << endl;
               return altezza_min_l + 1;
           }
           else if (altezza_min_l > altezza_min_r) 
		      {
			    cout << "altezza minimale " << h->data.key() << " " << altezza_min_r+1 << endl; 
				return altezza_min_r + 1;
		      }
		      ...
		      {
			    cout << "altezza minimale " << h->data.key() << " " << altezza_min_l+1 << endl;
				return altezza_min_l + 1;
		      }
		  }
	  }
    ...
    public:
		...
		void stampa_altezze_minimali()
		{
			altezza_minimale(root);
		}
