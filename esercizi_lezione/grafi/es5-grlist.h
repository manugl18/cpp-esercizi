...
inline ostream& operator << (ostream& s, Edge e)
...
public:
  ...
  void reverse() { 
	  ...
	  for (int i = 0; i < numVertex; i++)
	  {
		  int curpos = vertex[i]->currPos();
		  vertex[i]->moveToStart();
		  for (int j = 0; j < curpos; j++)
		  {
			  Edge temp = vertex[i]->getValue();
			  Edge newEdge(i, temp.weight());
			  vertex[temp.vertex()]->append(newEdge);
			  vertex[i]->remove();
		  }
	  }
	  // mantengo le liste di adiacenza ordinate per indice crescente
	  for (int i = 0; i < numVertex; i++)
	   selectionsort(vertex[i]);
  }
