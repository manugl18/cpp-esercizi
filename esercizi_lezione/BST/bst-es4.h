...
class BST 
  ...
  void rand_insertR(link& h, Item x)
   ...
   void inseriscilistapariR(link h, int livello, LList<Item>& L1)
   {
	   if (h==NULL) return;
	   if (livello%2 == 0) // livello pari
	   {
		   h->item.show(cout);
		   L1.append(h->item);
	   }
	   inseriscilistapariR(h->l, livello+1, L1);
	   inseriscilistapariR(h->r, livello+1, L1);
	   return;
   }
   /*
   int height(link h)
   ...
   */
	public:
	  ...
	  void inseriscilistapari(LList<Item>& L1)
	  {
		  inseriscilistapariR(head, 0, L1);
	  }
