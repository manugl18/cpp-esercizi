...
private:
	...
	void search_all_keysR(link h, Key v)
	{
		if (h == 0) return;
		Key t = h->item.key();
		if (v == t) // nodo uguale trovato.
		{
			h->item.show(cout);
			search_all_keysR(h->r, v); // un possibile nodo uguale si trova nel sottoalbero destro
			return;
		}
		if (v < t) return search_all_keysR(h->l, v); // Se la chiave che cerco (v) è più piccola, scendo nel ramo sinistro (h->l).
		else return search_all_keysR(h->r, v); // altrimenti vado a destra
	}
